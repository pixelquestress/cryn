...
#define MAX_ANIMATED_SYMBOLS 100
#define MAX_WALKABLE_SYMBOLS 256
...
public:
	...
	void SetTileAnimation(int Start, int End);
	void SetTileWalkable(bool w);
	BOOL IsAnimated();
	BOOL IsWalkable();
	int GetTileStart();
	int GetTileEnd();
...
public:
	...
	BOOL Walkable; // flag if tile is walkable or not.
	BOOL Animated; // flag if tile is animated.
	int TileStart; // tile # of starting animation.
	int TileEnd;   // tile # of ending animation.
};
...
public:
	...
	BOOL LoadAscii(const char* szFilename, int width, int height);
	BOOL SaveAscii(const char* szFilename);
	int GetScreenMapPosX2();
	int GetScreenMapPosY2();
	bool IsAnimated(int MapX, int MapY);
	bool IsWalkable(int MapX, int MapY);
	void AnimateTile(int MapX, int MapY);
	void AnimateMap(int delay);
...
public:
	...
	SymbolStruct AnimatedSymbols[MAX_ANIMATED_SYMBOLS];
	int AnimatedSymbolIndex = 0;

	char WalkableSymbols[MAX_WALKABLE_SYMBOLS];
	int WalkableSymbolIndex = 0;
