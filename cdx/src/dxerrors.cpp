...
//////////////////////////////////////////////////////////////////////////////////
#ifdef SAVE_RCSID
static char rcsid[] = "@(#) $Id: dxerrors.cpp,v 1.1.1.1 2000/04/22 16:13:58 hebertjo Exp $";
#endif
...
void CDXError( CDXScreen *Screen, char *str, ... )
{
    ...
    // Convert cp to a wide string
    std::wstring wideCp = ConvertToWideString(cp);

    if(Screen != NULL)
    {
        Screen->GetDD()->FlipToGDISurface();
        MessageBox((HWND)Screen->GetWindowHandle(), wideCp.c_str(), L"Error", MB_OK | MB_ICONERROR);
    }
    else
    {
        MessageBox(NULL, wideCp.c_str(), L"Error", MB_OK | MB_ICONERROR);
    }
	PostQuitMessage(0);
}
...
void DDError(HRESULT hErr, void* hWnd, LPCSTR szFile, int iLine)
{
    ...
    // Convert cp to a wide string
    std::wstring wideCp = ConvertToWideString(err);
    MessageBox((HWND)hWnd, wideCp.c_str(), L"Error", MB_OK);
	PostQuitMessage(0);
}
...
void DSError(HRESULT hErr, void* hWnd)
{
    ...
    // Convert cp to a wide string
    std::wstring wideCp = ConvertToWideString(err);
	int ret = MessageBox( (HWND)hWnd, wideCp.c_str(), L"Error", MB_YESNO );
...
////////////////////////////////////////////////////////////////////////
const char* DXErrorToString(HRESULT hErr)
{
  switch(hErr) 
  {
    case DD_OK:
