//////////////////////////////////////////////////////////////////////////////////
// $Source: /cvsroot/cdx/cdx3.0/src/cdx/ddutils.cpp,v $
// $Author: istan $
//
// $Log: ddutils.cpp,v $
...
//////////////////////////////////////////////////////////////////////////////////
#ifdef SAVE_RCSID
static char rcsid[] = "@(#) $Id: ddutils.cpp,v 1.5 2001/07/24 03:57:20 istan Exp $";
#endif
...
// Function to convert a wide string to a narrow string
std::string ConvertToNarrowString(const wchar_t* wideStr)
{
    size_t newSize = wcslen(wideStr) + 1;
    char* narrowStr = new char[newSize];
    size_t convertedChars = 0;
    wcstombs_s(&convertedChars, narrowStr, newSize, wideStr, _TRUNCATE);
    std::string result(narrowStr);
    delete[] narrowStr;
    return result;
}
...
BOOL WINAPI DDEnumCallback(GUID* lpGUID, LPWSTR lpDesc, LPWSTR lpName, LPVOID lpContext, HMONITOR hm)
#else
...
#endif
{
    ...
    std::string narrowName = ConvertToNarrowString(lpName);
    std::string narrowDesc = ConvertToNarrowString(lpDesc);

	strcpy( Drivers->lpName, narrowName.c_str());
    strcpy(Drivers->lpDesc, narrowDesc.c_str());
...
extern std::wstring ConvertToWideString(const char* narrowStr);
...
extern "C" CDX_LPDIRECTDRAWSURFACE DDLoadBitmap(CDX_LPDIRECTDRAW pdd, LPCSTR szBitmap, int dx, int dy, BOOL memoryType)
{
    ...
    std::wstring wideBitmap = ConvertToWideString(szBitmap);

    //
    //  try to load the bitmap as a resource, if that fails, try it as a file
    //
    hbm = (HBITMAP)LoadImage(GetModuleHandle(NULL), wideBitmap.c_str(), IMAGE_BITMAP, dx, dy, LR_CREATEDIBSECTION);

    if (hbm == NULL)
        hbm = (HBITMAP)LoadImage(NULL, wideBitmap.c_str(), IMAGE_BITMAP, dx, dy, LR_LOADFROMFILE|LR_CREATEDIBSECTION);
...
CDX_LPDIRECTDRAWSURFACE DDLoadSizeBitmap(CDX_LPDIRECTDRAW pdd, LPCSTR szBitmap, int *dx, int *dy, BOOL memoryType)
{
	...
	hbm = (HBITMAP)LoadImage(GetModuleHandle(NULL), wideBitmap.c_str(), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

	if (hbm == NULL)
	hbm = (HBITMAP)LoadImage(NULL, wideBitmap.c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE|LR_CREATEDIBSECTION);
								 ...
								 LPCSTR szBitmap)
{
    ...
    *ppPalette = NULL;
    ...
    //  Try to load the bitmap as a resource, if that fails, try it as a file
    hResource = FindResource( NULL, wideBitmap.c_str(), RT_BITMAP );
    if( hResource )
    {
    ...
    hFile = CreateFile(wideBitmap.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL );
