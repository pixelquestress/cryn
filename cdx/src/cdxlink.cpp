//////////////////////////////////////////////////////////////////////////////////
// $Source: /cvsroot/cdx/cdx3.0/src/cdx/cdxlink.cpp,v $
// $Author: hebertjo $
//
// $Log: cdxlink.cpp,v $
...
//////////////////////////////////////////////////////////////////////////////////
#ifdef SAVE_RCSID
static char rcsid[] = "@(#) $Id: cdxlink.cpp,v 1.1.1.1 2000/04/22 16:10:25 hebertjo Exp $";
#endif
...
{
	LPWSTR Name = (LPWSTR)lpContext;
	LPWSTR Data = NULL;

	// Convert lpData to wide string
	int dataSize = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)lpData, -1, NULL, 0);
	Data = new wchar_t[dataSize];
	MultiByteToWideChar(CP_ACP, 0, (LPCSTR)lpData, -1, Data, dataSize);
...
{
	// Create the DirectPlay object
	HRESULT rval = CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_INPROC_SERVER,
		IID_IDirectPlay3A, (LPVOID*)&m_DP);
	if (rval != DP_OK) return FALSE;
	...
	// Convert IPAddress to wide string
	int ipSize = MultiByteToWideChar(CP_ACP, 0, IPAddress, -1, NULL, 0);
	wchar_t* wIPAddress = new wchar_t[ipSize];
	MultiByteToWideChar(CP_ACP, 0, IPAddress, -1, wIPAddress, ipSize);

	Address[1].guidDataType = DPAID_INet;
	Address[1].dwDataSize = lstrlenW(wIPAddress) + 1;
	Address[1].lpData = wIPAddress;
...
BOOL CDXLink::CreateModem(void* hWnd, char* PhoneNo)
{
	...
	rval = m_DPLobby->EnumAddress(EnumModemAddress, ModemAddress, AddressSize, (LPVOID)ModemName);
	if(rval != DP_OK) return FALSE;

	// Convert ModemName to wide string
	int modemNameSize = MultiByteToWideChar(CP_ACP, 0, ModemName, -1, NULL, 0);
	wchar_t* wModemName = new wchar_t[modemNameSize];
	MultiByteToWideChar(CP_ACP, 0, ModemName, -1, wModemName, modemNameSize);

	// Convert PhoneNo to wide string
	int phoneNoSize = MultiByteToWideChar(CP_ACP, 0, PhoneNo, -1, NULL, 0);
	wchar_t* wPhoneNo = new wchar_t[phoneNoSize];
	MultiByteToWideChar(CP_ACP, 0, PhoneNo, -1, wPhoneNo, phoneNoSize);

	Address[1].guidDataType = DPAID_Modem;
	Address[1].dwDataSize = lstrlenW(wModemName) + 1;
	Address[1].lpData = (LPVOID)ModemName;

	Address[2].guidDataType = DPAID_Phone;
	Address[2].dwDataSize = lstrlenW(wPhoneNo) + 1;
